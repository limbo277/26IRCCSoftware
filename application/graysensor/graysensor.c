...
static  GrayscaleData_t grayscale_data_KF = {0}; // 灰度传感器数据的卡尔曼滤波后值
...
void Graysensor_Kalman_Init() {
        ...
        Gray_KF[i].StateMinVariance[0] =
                0.01f; // 最小方差抑制过度收敛
        Gray_KF[i].UseAutoAdjustment =
                0; // 关闭自动调整（简单场景）
        }
}
void Graysensor_Kalman_Filter() {
  for (int i = 0; i < 8; i++) {
    ...
    grayscale_data_KF.sensor_values[i] = (uint16_t)Kalman_Filter_Update(&Gray_KF[i]);
  }
}
